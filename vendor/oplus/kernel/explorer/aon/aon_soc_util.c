			...
			int gpio_en)
{
	...
	if (!gpio_tbl || !size) {
		pr_err("%s invalid gpio_tbl %pK / size %d", __func__,
			gpio_tbl, size);
		return -EINVAL;
	}
...
static int aon_soc_util_get_dt_regulator_info(struct aon_sensor_ctrl_t *s_ctrl)
{
	...
	pr_err("%s %d regulators found", __func__, count);
	if (count != -EINVAL) {
		if (count <= 0) {
...
static int aon_soc_util_get_dt_clk_info(struct aon_sensor_ctrl_t *s_ctrl)
{
	struct device_node *of_node = NULL;
	int count = 0;
	int num_clk_rates = 0, num_clk_levels = 0;
	int i = 0, j = 0, rc = 0;
	...
	u16 gpio_array_size)
{
	...
	if (!val_array)
		return -ENOMEM;
	pr_info("%s val_array mm-kcalloc-size: %lld",
	...
	pr_info("%s gconf->cam_gpio_req_tbl mm-kcalloc-size: %lld",
		__func__, count * sizeof(struct gpio));
	...
	kfree(val_array);
	pr_info("%s val_array mm-kfree", __func__);

	return rc;

free_gpio_req_tbl:
	kfree(gconf->cam_gpio_req_tbl);
	pr_info("%s gconf->cam_gpio_req_tbl mm-kfree", __func__);
free_val_array:
	kfree(val_array);
	pr_info("%s val_array mm-kfree", __func__);
...
static int aon_soc_util_get_gpio_info(struct aon_sensor_ctrl_t *s_ctrl)
{
	...
	pr_info("%s gpio_array mm-kcalloc-size: %lld",
		__func__, gpio_array_size * sizeof(u16));
	...
	pr_info("%s gconf mm-kzalloc-size: %lld",
		__func__, sizeof(*gconf));
	...
	pr_info("%s gconf->cam_gpio_common_tbl mm-kcalloc-size: %lld",
		__func__, gpio_array_size * sizeof(struct gpio));
	...
	kfree(gpio_array);
	pr_info("%s gpio_array mm-kfree", __func__);
...
free_gpio_conf:
	kfree(gconf);
	pr_info("%s gconf mm-kfree", __func__);
	soc_info->gpio_data = NULL;

	return rc;
}

static int aon_cx_ipeak_register_cx_ipeak(struct aon_hw_soc_info *soc_info)
{
	int rc = 0;
	return rc;
}
...
static int aon_soc_util_get_dt_properties(struct aon_sensor_ctrl_t *s_ctrl)
{
	...
	if (of_find_property(of_node, "qcom,cam-cx-ipeak", NULL))
		rc = aon_cx_ipeak_register_cx_ipeak(soc_info);
	...
	struct aon_camera_gpio_num_info **pgpio_num_info)
{
	...
	if (!s_ctrl || !pgpio_num_info) {
		pr_err("%s device node NULL, s_ctrl %pK, pgpio_num_info %pK\n",
	...
	*pgpio_num_info = kzalloc(sizeof(struct aon_camera_gpio_num_info),
	...
	pr_info("%s gpio_num_info mm-kzalloc-size: %lld",
		__func__, sizeof(struct aon_camera_gpio_num_info));
...
free_gpio_info:
	kfree(gpio_num_info);
	pr_info("%s gpio_num_info mm-kfree", __func__);
	gpio_num_info = NULL;
	return rc;
}
	...
	struct aon_sensor_board_info *s_info)
{
	...
	if (!of_node || !s_info) {
		pr_err("%s invalid params, of_node %pK, s_info %pK",
...
static s32 aon_sensor_driver_get_dt_data(struct aon_sensor_ctrl_t *s_ctrl)
{
	...
	pr_info("%s s_ctrl->sensordata mm-kzalloc-size: %lld",
		__func__, sizeof(*sensordata));
...
free_sensor_data:
	kfree(sensordata);
	pr_info("%s sensordata mm-kfree", __func__);
	return rc;
}
...
static s32 aon_sensor_init_default_params(struct aon_sensor_ctrl_t *s_ctrl)
{
	/* Validate input parameters */
	if (!s_ctrl) {
		pr_err("aon failed: invalid params s_ctrl %pK", s_ctrl);
		return -EINVAL;
	}

	/* Initialize cci_client */
	s_ctrl->client = kzalloc(sizeof(struct cam_sensor_cci_client), GFP_KERNEL);
	if (!(s_ctrl->client)) {
		pr_err("%s kzalloc failed", __func__);
		return -ENOMEM;
	}
	pr_info("%s s_ctrl->client mm-kzalloc-size: %lld",
		__func__, sizeof(struct cam_sensor_cci_client));
	s_ctrl->client->cci_device = s_ctrl->cci_num;
	...
	u16 power_setting_size)
{
	...
	if (!soc_info || !power_setting) {
		pr_err("%s %d failed: soc_info %pK power_setting %pK",
...
s32 delete_request(struct i2c_settings_array *i2c_array)
{
	...
	list_for_each_entry_safe(i2c_list, i2c_next,
		...
		pr_info("%s i2c_list->i2c_settings.reg_setting mm-vfree", __func__);
		list_del(&(i2c_list->list));
		kfree(i2c_list);
		pr_info("%s i2c_list mm-kfree", __func__);
	}
		...
		struct aon_hw_soc_info *soc_info)
{
	...
	if (!ctrl || !soc_info) {
		pr_err("%s Invalid handle, ctrl %pK soc_info %pK",
			__func__, ctrl, soc_info);
		return -EINVAL;
	}
		...
		struct aon_hw_soc_info *soc_info)
{
	...
	pr_info("%s begin", __func__);
	if (!ctrl || !soc_info) {
		pr_err("%s failed, ctrl %pK, soc_info %pK", __func__, ctrl, soc_info);
		return -EINVAL;
	}
